...
<c:if test="${! empty sessionScope.id }">
   <c:if test="${sessionScope.id eq bb.name}">
      <input type="button" value="글수정" class="btn" onclick="location.href='<c:url value="/board/fupdate?num=${bb.num}"/>'">
      <input type="button" value="글삭제" class="btn" onclick="location.href='<c:url value="/board/fdelete?num=${bb.num}"/>'">
   </c:if>
</c:if>
